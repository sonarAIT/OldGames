...
public:
	int draw() {
		...
		if(EnemyArray[0].getShotFlag()){
			...
			EnemyBulletArray << EnemyBulletInstance;
			if(Gamemode == 1){
				EnemyBulletInstance.setXY((int)EnemyArray[0].getX(),(int)EnemyArray[0].getY(),atan2(ShipY-EnemyArray[0].getY(),ShipX-EnemyArray[0].getX())-0.436332);
				EnemyBulletArray << EnemyBulletInstance;
				EnemyBulletInstance.setXY((int)EnemyArray[0].getX(),(int)EnemyArray[0].getY(),atan2(ShipY-EnemyArray[0].getY(),ShipX-EnemyArray[0].getX())+0.436332);
				EnemyBulletArray << EnemyBulletInstance;
			}
	...
	void init(int Gamemode) {
		...
		Texture tex(Resource(U"File/ge5.png"));
		struct Enemy5 enemy;
		enemy.init(tex,30);
		enemy.setXY(900,500,Gamemode == 0 ? 500 : 750);
		EnemyArray << enemy;
